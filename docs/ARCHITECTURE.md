...
- **Which model, and how that was decided.** `gemini-3.1-pro-preview` — a preview name, deliberately: there is no GA Pro in the 3.x line, and the GA alternative is a generation older with a retirement date, which spends the reason for being on Pro at all. If the name is withdrawn the failure is the soft one described below and the fix is `EXTRACTION_MODEL`. Flash was measured against the real RFQ through the same instructions and the same guards and gave an identical answer — 45 lines, 27 exact, the struck-through size corrected on row 1 — for about a fiftieth of the cost. That is one document, and the one this was written against; the readings that matter are the ones not yet in front of it, where a wrong answer arrives as a plausible schedule rather than an error. The default buys headroom there. `EXTRACTION_MODEL="gemini-3.5-flash"` takes the saving back without a deploy, and anything else is a model this document has never been put in front of.
