...
- **Measured, on a real customer RFQ.** 48 rows offered, 45 kept, 27 matching a costed item code exactly — against 0 lines from the pattern reader on the same document. The three refusals were a row with no quantity printed and two the customer marked nought. Each of the 18 remaining Partials states its own reason: a real gap in the library, or a field the document does not print.
