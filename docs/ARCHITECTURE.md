...
- **Which model, and how that was decided.** `gemini-3.5-flash`. The first working version ran on a frontier model at roughly a dollar a document, which is a real cost on something that runs at every upload. So the same file was read again through the same instructions and the same guards: identical answer — 45 lines, 27 exact, the struck-through size corrected on row 1 — at about a fiftieth of the cost. `EXTRACTION_MODEL` overrides it without a deploy, and anything but the default is a model nobody has measured this document against.
